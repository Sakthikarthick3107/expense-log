...
class _CollectionFormModalState extends State<CollectionFormModal> {
  ...
  void initState() {
    ...
    addedExpenseNotifier.value =
        List.from(widget.collection?.expenseList ?? []);
  }
                      ...
                      IconButton(
                          onPressed: () {
                            WarningDialog.showWarning(
                                ...
                                title: 'Warning',
                                message:
                                    'Are you sure to delete collection ${widget.collection?.name}?',
                                onConfirmed: () {
                                  _collectionService
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            trailing: IconButton(
                              onPressed: () {
                                addedExpenseNotifier.value = List.from(
                                    addedExpenseNotifier.value
                                      ..removeAt(index));
                              },
                ...
                SizedBox(
                  ...
                  child: TextButton(
                    onPressed: () async {
                      ...
                      if (addExpense != null) {
                        addedExpenseNotifier.value = List.from(
                            addedExpenseNotifier.value..add(addExpense));
                      }
                    },
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState?.validate() ?? false) {
                        ...
                        Collection newCollection = Collection(
                            id: widget.collection?.id ??
                                await _settingsService
                                    .getBoxKey('collectionId'),
                            name: name,
                            description: description,
                            expenseList: expenseList,
                            created:
                                widget.collection?.created ?? DateTime.now(),
                        ...
                        final action =
                            widget.collection != null ? 'edited' : 'created';
                        if (status == 1) {
                          Navigator.pop(context);
                    ...
                    child: Text(
                        '${widget.collection != null ? 'Edit' : 'Create'} Collection'),
                  ),
                ),
                SizedBox(
                  height: 10,
